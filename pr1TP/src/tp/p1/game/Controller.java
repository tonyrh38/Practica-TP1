...
public class Controller {
	...
	public void run() {
		...
		while(!this.game.playerDefeated() && !this.game.enemyDefeated() ) {
			...
			this.computerAction();
			this.update();	
			//TODO: revisar comando que hacen acciones. Intenta actualizar cosas que no existen
		}
	...
	private void userCommand() {
		...
		do {
			...
			if((words[0].equals("move") || words[0].equals("m")) && words.length == 3) {
				...
				System.out.print("Comando err�neo o no reconocido. ");
				command = false;
			}
		}while(!command);
